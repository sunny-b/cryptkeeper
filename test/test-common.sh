...
test() {
  ...
  test_string="### Testing $1 - SHELL: $TARGET_SHELL"
  if [ -n "$TARGET_ENCRYPTION" ]; then
    test_string="$test_string - ENCRYPTION: $TARGET_ENCRYPTION"
  fi
...
source "$TEST_DIR/scenarios/utils.sh"
ck_env

test base

algorithms=("aes" "ecc" "serpent" "rsa")
for algo in "${algorithms[@]}"; do
  TARGET_ENCRYPTION="$algo" test encryption
done

test direnv
test direnv-integrate
test nested
test direnv-nested
#   echo "Setting up"
#   direnv_eval
